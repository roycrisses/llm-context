...
def main(
    ...
    summary = f"({total_tokens:,} tokens from {len(trimmed)} files)"

    if output:
        try:
            output.write_text(context_block, encoding="utf-8")
            _echo_success(f"Context {summary} saved to '{output}'.")
    ...
    if do_copy:
        try:
            ...
            _echo_success(f"Context {summary} copied to clipboard.")
        except ImportError:
            _echo_error(
