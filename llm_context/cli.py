...
def main(
    ...
    """
    Analyze DIRECTORY (defaults to '.'), find files relevant to your --ask
    question, and assemble a ready-to-use LLM context block.
    ...
    # ── 5. Output ───────────────────────────────────────────────────────────
    total_tokens = count_tokens(context_block, model=model)

    if output:
        try:
            output.write_text(context_block, encoding="utf-8")
            _echo_success(
                f"Context saved to '{output}' ({len(trimmed)} files, {total_tokens:,} tokens)."
            )
        ...
        _echo_info(f"Context printed to stdout ({len(trimmed)} files, {total_tokens:,} tokens).")
    ...
    if do_copy:
        try:
            ...
            _echo_success(
                f"Context copied to clipboard ({len(trimmed)} files, {total_tokens:,} tokens)."
            )
        except ImportError:
