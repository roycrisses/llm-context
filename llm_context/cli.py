...
def main(
    ...
    summary_parts = [f"Included {len(trimmed)} file(s)"]
    if num_truncated > 0:
        summary_parts.append(f"{num_truncated} truncated")
    if num_omitted > 0:
        summary_parts.append(f"{num_omitted} omitted")

    summary = f"{', '.join(summary_parts)}. Total: {tokens:,} tokens."

    # Always echo summary to stderr for immediate feedback
    _echo_info(summary)
